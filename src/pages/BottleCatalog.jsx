...
import { ArrowLeft, ShoppingBag, Plus, Minus, CreditCard, CheckCircle2, ShieldCheck, MapPin, Zap } from 'lucide-react';
import { bambooMenu } from '../data/bambooData';
...
export function BottleCatalog() {
  ...
  const [category, setCategory] = useState(bambooMenu[0].category);
  ...
  const total = Object.entries(cart).reduce((acc, [name, qty]) => {
     let b; bambooMenu.forEach(cat => cat.items.forEach(itm => { if(itm.name === name) b = itm; }));
     return acc + (b ? b.price * qty : 0);
  }, 0);

  const totalItems = Object.values(cart).reduce((a,b)=>a+b,0);
  const activeItems = bambooMenu.find(c => c.category === category)?.items || [];
  ...
  const handlePayment = async () => {
    ...
    try {
      await placeOrder({
        userId: 'mario_rossi', // Should come from useAuth
        items: orderItems,
        total,
        paymentMethod,
        target: deliveryTarget,
        otherTable: deliveryTarget === 'other' ? otherTable : null,
        table: 'Console VIP 14',
        pr: 'Sara'
      });
  ...
  if(isPaid) {
      return (
         <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '2rem', flex: 1, textAlign: 'center' }}>
            ...
            <p style={{ color: 'var(--text-secondary)', marginBottom: '2rem' }}>Ordine Inviato (Tavolo Console VIP 14 - PR: Sara). La Cassa sta preparando la tua comanda.</p>
            ...
            <Button variant="primary" style={{ width: '100%' }} onClick={() => navigate('/customer/profile')}>Vai ai Miei Ordini</Button>
         </div>
      );
  }
         ...
         <div style={{ display: 'flex', flexDirection: 'column', padding: '1rem', flex: 1 }}>
                ...
                {deliveryTarget === 'me' ? (
                  <div style={{ background: 'rgba(255,255,255,0.05)', padding: '1rem', borderRadius: '12px' }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                          <p style={{ fontWeight: 600, margin: 0 }}>Tavolo Assegnato: Console VIP 14</p>
                  ...
                  <div style={{ background: 'linear-gradient(135deg, rgba(255,105,180,0.15), rgba(124,58,237,0.15))', padding: '1rem', borderRadius: '12px', border: '1px solid rgba(255,105,180,0.3)' }}>
                      ...
                      <input type="text" placeholder="Es. Tavolo Pista 4" className="input-base" value={otherTable} onChange={e => setOtherTable(e.target.value)} style={{ background: 'rgba(0,0,0,0.5)', border: '1px solid rgba(255,255,255,0.1)', padding: '0.75rem', fontSize: '0.9rem' }} />
                      <p style={{ fontSize: '0.75rem', color: 'var(--text-secondary)', marginTop: '0.75rem', margin: 0 }}>Il cameriere consegnerà la bottiglia al tavolo indicato a tuo nome. Un'ottima mossa per rompere il ghiaccio. 🔥</p>
                  </div>
                )}
            </Card>
            ...
            <Card style={{ marginBottom: '1.5rem' }}>
                ...
                {Object.entries(cart).map(([name, qty]) => {
                    let b; bambooMenu.forEach(c => c.items.forEach(i => { if(i.name === name) b = i; }));
                    return (
            ...
            <Button disabled={!hasEntered} variant={hasEntered ? 'primary' : 'secondary'} onClick={handlePayment} style={{ width: '100%', padding: '1.2rem', fontSize: '1.1rem', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.5rem', opacity: hasEntered ? 1 : 0.5 }}>
