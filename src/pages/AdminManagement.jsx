import { streamOrders, validateBottleQR, streamSystemState, updateSystemState, setDoc, doc, db } from '../services/db';
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', padding: '1rem', flex: 1, paddingBottom: '2rem' }}>
       <header style={{ display: 'flex', gap: '1rem', alignItems: 'center', marginBottom: '2rem' }}>
        ...
        <Card style={{ marginBottom: '1.5rem', background: hackerMode ? 'rgba(255,59,92,0.1)' : 'rgba(124,58,237,0.05)', border: hackerMode ? '1px solid var(--error)' : '1px solid var(--accent-light)' }}>
           <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <div>
                 <h3 style={{ margin: 0, display: 'flex', alignItems: 'center', gap: '0.5rem', color: hackerMode ? 'var(--error)' : 'var(--accent-light)' }}>
                    <Zap size={20} /> TERMINAL HACKER (Crowd Control)
                 </h3>
                 <p style={{ fontSize: '0.8rem', color: 'var(--text-secondary)', margin: '0.3rem 0 0 0' }}>{hackerMode ? 'DISTURBO SEGNALE ATTIVO - Gli ospiti stanno ballando' : 'Sincronizzazione segnale standard'}</p>
              </div>
              <Button 
                variant={hackerMode ? 'primary' : 'secondary'} 
                onClick={() => updateSystemState({ hackerMode: !hackerMode })}
                style={{ background: hackerMode ? 'var(--error)' : '', borderColor: hackerMode ? 'var(--error)' : '' }}
              >
                {hackerMode ? <Wifi size={18} /> : <WifiOff size={18} />} {hackerMode ? 'RIPRISTINA' : 'HACKER MODE'}
              </Button>
           </div>
        </Card>
