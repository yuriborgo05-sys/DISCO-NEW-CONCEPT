...
import { Sparkles, ArrowLeft, GlassWater, ShieldCheck, HelpCircle } from 'lucide-react';
      ...
      {showPopup && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.85)', backdropFilter: 'blur(10px)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '1.5rem', animation: 'fadeIn 0.3s ease' }}>
           <div style={{ background: 'linear-gradient(180deg, rgba(16, 185, 129, 0.4) 0%, #111 100%)', padding: '2rem', borderRadius: '24px', textAlign: 'center', border: '1px solid var(--success)', width: '100%', maxWidth: '400px', boxShadow: '0 0 50px rgba(16, 185, 129, 0.2)' }}>
             ...
             <p style={{ color: 'rgba(255,255,255,0.8)', marginBottom: '1.5rem', lineHeight: 1.5 }}>Essendo la tua prima volta al TOO MUCH, ti abbiamo accreditato un <strong style={{color: 'var(--success)'}}>Drink Omaggio Digitale</strong> nel profilo!<br/><br/>Lo troverai nella tua area personale.</p>
       ...
       <Card style={{ padding: '1.5rem', display: 'flex', flexDirection: 'column', gap: '1rem' }}>
         ...
         <select className="input-base" value={formData.occupation} onChange={e => update('occupation', e.target.value)}>
          ...
          <select className="input-base" value={formData.role} onChange={e => update('role', e.target.value)} style={{ border: '1px solid var(--success)' }}>
             <option value="user" style={{color: 'black'}}>Cliente VIP</option>
             <option value="pr" style={{color: 'black'}}>PR (Promoter)</option>
             <option value="head_pr" style={{color: 'black'}}>Capo PR (Manager)</option>
             <option value="image_girl" style={{color: 'black'}}>Ragazza Immagine (Hostess)</option>
             <option value="photographer" style={{color: 'black'}}>Fotografo Ufficiale</option>
             <option value="bodyguard" style={{color: 'black'}}>Security (Bodyguard)</option>
             <option value="cashier" style={{color: 'black'}}>Cassa (Pagamenti)</option>
             <option value="waiter" style={{color: 'black'}}>Cameriere (Service)</option>
             <option value="admin" style={{color: 'black'}}>Regia / Direzione Artistica</option>
          </select>
